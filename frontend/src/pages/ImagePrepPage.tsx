...
export function ImagePrepPage() {
  ...
  async function loadPrep() {
    ...
    if (g.aspect_ratio === "16:9" && (g.crop_x ?? 0.5) === 0.5 && (g.crop_y ?? 0.5) === 0.5 && (g.exposure ?? 0) === 0 && (g.brightness ?? 0) === 0 && (g.contrast ?? 0) === 0 && (g.gamma ?? 1) === 1) state = await setGlobalImageTransform(project.id, revision.id, { ...g, aspect_ratio: "source" });
    setPrep(state); setGlobalDraft(state.global_transform || { aspect_ratio: "source" }); setResolutionDraft(state.training_resolution);
