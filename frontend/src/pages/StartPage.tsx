...
export function StartPage() {
  ...
  return <div className="stack">
    ...
    {!project && recentProjects.length > 0 && <section className="panel stack"><div className="prep-section-heading"><div><div className="card-title">Recent Projects</div><p className="muted">Removing an item here only clears it from this browser's recent list; project files are not deleted.</p></div><button className="secondary" onClick={clearRecentProjects}>Clear recent projects</button></div><div className="project-list">{recentProjects.map((item) => <div className="project-row-shell" key={item.id}><button className="project-row" onClick={() => openProject(item)} disabled={loading}><span><strong>{item.name}</strong><small>{item.external_source.path}</small></span><span className="muted">{item.dataset_revisions.length} datasets · {item.runs.length} runs</span></button><button className="recent-remove" onClick={() => dismissRecent(item.id)} title="Remove from recent projects" aria-label={`Remove ${item.name} from recent projects`}>×</button></div>)}</div></section>}
