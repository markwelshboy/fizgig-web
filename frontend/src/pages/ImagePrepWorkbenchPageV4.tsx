...
function mergedTransform(globalTransform: ImageTransform, asset: ProjectAsset) {
  return { ...inheritedTransform(globalTransform, asset), ...(asset.transform_override || {}) };
}
...
export function ImagePrepWorkbenchPageV4() {
  ...
  const derivedOverrides = derivatives.filter((asset) => transformSummary(asset.transform_override || {}, asset.baked_transform ? derivativeBaseTransform(asset) : {}).length > 0);
  ...
  async function inclusion(value: boolean) {
    ...
    setBusy(true); try { await setImageInclusion(project.id, revision.id, [...selected], value); setSelected(new Set()); await refresh(); notify(`${count} source image(s) ${value ? "included" : "excluded"}.`); } finally { setBusy(false); }
  }
  ...
  async function removeDerivative(asset: ProjectAsset) {
    ...
    if (!window.confirm(`Remove project derivative ${asset.filename}?\n\nThe external source is unaffected.`)) return;
    setBusy(true);
    try {
      await deleteDerivativeAsset(project.id, revision.id, asset.filename);
      if (focusName === asset.filename) setFocusName("");
      if (editName === asset.filename) { setEditName(""); setOverrideDraft({}); }
      await refresh();
      notify(`${asset.filename} removed from this project.`);
    } catch (error) {
      notify(error instanceof Error ? error.message : "Unable to remove derivative", "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function resetOverride() {
    ...
    setBusy(true); try { await setAssetImageTransform(project.id, revision.id, editAsset.filename, reset); setOverrideDraft(reset); await refresh(); notify(editAsset.baked_transform ? "Derivative adjustment reset to its baked prepared image." : "Image override reset."); } finally { setBusy(false); }
  }
  ...
  return (
    <div className="stack image-prep-page image-prep-v3 image-prep-v4">
      ...
      <section className="panel stack">
        ...
        {derivatives.length > 0 && <><div className="asset-subheading">Project derivatives <span>{faceDerived.length} face · {manualDerived.length} manual</span></div><div className="prep-grid derivative-grid">{derivatives.map((asset) => { const changes = transformSummary(asset.transform_override || {}, asset.baked_transform ? derivativeBaseTransform(asset) : {}); return <div className="derivative-card-shell" key={asset.filename}><button className={`prep-image-card ${focusName === asset.filename ? "focused" : ""}`} onClick={() => setFocusName(asset.filename)}><div className="prep-image-wrap"><img src={preparedAssetPreviewUrl(project.id, revision.id, asset.filename)} alt={asset.filename} />{changes.length > 0 && <span className="transform-badge">Adjusted</span>}<span className="derived-badge">{asset.origin === "face" ? "Face" : "Manual"}</span></div><div className="prep-image-meta"><strong>{asset.filename}</strong><span>Parent: {asset.parent_filename || "source asset"}</span>{changes.length > 0 && <span className="transform-state-line"><span className="changed">{changes.join(" · ")}</span></span>}</div></button><button className="derivative-delete" disabled={busy} title={`Remove ${asset.filename}`} aria-label={`Remove ${asset.filename}`} onClick={() => removeDerivative(asset)}>×</button></div>; })}</div></>}
      ...
      <section className="panel stack" id="per-image-v4">
        <div className="prep-section-heading"><div><p className="eyebrow">Stage 5</p><div className="card-title">Per-image Exceptions & Crop Review</div><p className="muted">Collapsed cards use the exact saved prepared preview. Open one to move/resize its crop or tune it further.</p></div><span className="muted">{included.length} working · {sourceOverrides.length} source overrides · {derivedOverrides.length} adjusted derivatives</span></div>
        <div className="processed-grid">{included.map((asset) => { const image = dataset.images.find((candidate) => candidate.filename === asset.filename); if (!image) return null; const active = editName === asset.filename; const base = inheritedTransform(globalDraft, asset); const effective = mergedTransform(globalDraft, asset); const explicitChanges = transformSummary(asset.transform_override || {}, base); const cardLabel = asset.asset_kind === "derived" ? `${asset.origin === "face" ? "Face" : "Manual"} derivative` : explicitChanges.length ? "Image override" : "Global recipe"; return <div className="processed-grid-item" key={asset.filename}><button className={`processed-card ${active ? "selected" : ""}`} onClick={() => openEditor(asset.filename)}><div className="processed-image fixed-review-card"><img src={preparedAssetPreviewUrl(project.id, revision.id, asset.filename)} alt={asset.filename} />{explicitChanges.length > 0 && <span className="transform-badge">Adjusted</span>}</div><div className="prep-image-meta"><strong>{asset.filename}</strong><span>{cardLabel}</span>{explicitChanges.length > 0 && <span className="transform-state-line"><span className="changed">{explicitChanges.join(" · ")}</span></span>}</div></button>{active && <div className="inline-image-editor crop-review-editor"><PositionCropEditor imageUrl={image.image_url} filename={asset.filename} aspect={overrideDraft.aspect_ratio ?? base.aspect_ratio ?? targetAspect} scale={overrideDraft.crop_scale ?? base.crop_scale ?? 1} cropX={overrideDraft.crop_x ?? base.crop_x ?? 0.5} cropY={overrideDraft.crop_y ?? base.crop_y ?? 0.5} filter={previewFilter({ ...base, ...overrideDraft })} onChange={(next) => setOverrideDraft({ ...overrideDraft, crop_x: next.x, crop_y: next.y, crop_scale: next.scale, crop_mode: "fill" })} /><div className="stack inline-editor-controls"><div><p className="eyebrow">Per-image exception</p><div className="card-title">{asset.filename}</div></div><label>Aspect<select value={overrideDraft.aspect_ratio ?? ""} onChange={(event) => setOverrideDraft({ ...overrideDraft, aspect_ratio: event.target.value || undefined, crop_x: 0.5, crop_y: 0.5, crop_scale: 1, crop_mode: event.target.value && event.target.value !== "source" ? "fill" : undefined })}><option value="">Inherit {base.aspect_ratio || targetAspect}</option>{ASPECTS.map((aspect) => <option key={aspect} value={aspect}>{aspect === "source" ? "Source / no crop" : aspect}</option>)}</select></label><div className="tuning-grid"><SliderField label="Exposure" min={-2} max={2} step={0.1} value={overrideDraft.exposure ?? base.exposure ?? 0} onChange={(value) => setOverrideDraft({ ...overrideDraft, exposure: value })} /><SliderField label="Brightness" min={-0.5} max={0.5} step={0.05} value={overrideDraft.brightness ?? base.brightness ?? 0} onChange={(value) => setOverrideDraft({ ...overrideDraft, brightness: value })} /><SliderField label="Contrast" min={-0.5} max={0.5} step={0.05} value={overrideDraft.contrast ?? base.contrast ?? 0} onChange={(value) => setOverrideDraft({ ...overrideDraft, contrast: value })} /><SliderField label="Gamma" min={0.5} max={1.5} step={0.05} value={overrideDraft.gamma ?? base.gamma ?? 1} onChange={(value) => setOverrideDraft({ ...overrideDraft, gamma: value })} /></div><div className="actions"><button className="secondary" onClick={resetOverride}>Reset to prepared base</button>{asset.asset_kind === "derived" && <button className="danger" disabled={busy} onClick={() => removeDerivative(asset)}>Remove derivative</button>}<button className="primary" disabled={!overrideDirty || busy} onClick={saveOverride}>{overrideDirty ? "Save image override" : "Image override saved"}</button></div></div></div>}</div>; })}</div>
      </section>
