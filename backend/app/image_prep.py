...
class ImagePrepStore:
    def _load(self, project_id: str, revision_id: str) -> tuple[Path, dict[str, Any]]:
        ...
        if "global_transform" not in manifest:
            manifest["global_transform"] = dict(DEFAULT_GLOBAL_TRANSFORM); changed = True
        if "training_resolution" not in manifest:
            manifest["training_resolution"] = dict(DEFAULT_TRAINING_RESOLUTION); changed = True
    ...
    def state(self, project_id: str, revision_id: str) -> dict[str, Any]:
        ...
        for asset in included:
            ...
            if path.parent == files_dir and path.is_file():
                ...
                resolution_assets.append({"filename": asset.get("filename"), **_resolution_analysis(path, transform, policy)})
        return {
    ...
    def create_manual_crop(self, project_id: str, revision_id: str, filename: str, crop: dict[str, float], aspect_ratio: str) -> dict[str, Any]:
        ...
        with Image.open(source) as image:
            ...
            x = max(0.0, min(1.0, float(crop.get("x", 0.0)))); y = max(0.0, min(1.0, float(crop.get("y", 0.0))))
        ...
        asset = {"id": uuid.uuid4().hex[:16], "filename": output.name, "image_sha256": _sha256(output), "caption": "", "caption_sha256": hashlib.sha256(b"").hexdigest(), "origin": "manual_crop", "parent_asset_id": parent.get("id"), "parent_filename": filename, "asset_kind": "derived", "included": True, "transform_override": {"aspect_ratio": aspect_ratio}, "operations": [{"type": "manual_crop", "aspect_ratio": aspect_ratio, "normalized_crop": {"x": x, "y": y, "width": w, "height": h}, "pixel_box": list(box), "created_at": _now()}]}
    ...
    def append_operation(self, project_id: str, revision_id: str, filenames: list[str], operation: dict[str, Any]) -> dict[str, Any]:
        manifest_path, manifest = self._load(project_id, revision_id)
        wanted = set(filenames); found = []; stamp = {"recorded_at": _now(), **operation}
        for asset in manifest.get("assets", []):
            if asset.get("filename") in wanted:
    ...
    def materialize_run_dataset(self, project_id: str, revision_id: str, run: dict[str, Any]) -> dict[str, Any]:
        _, manifest = self._load(project_id, revision_id)
        source_dir = Path(manifest["files_path"]).resolve(); run_dir = Path(run["output_dir"]).resolve(); trainer_dir = run_dir / "dataset"
        if trainer_dir.exists(): shutil.rmtree(trainer_dir)
        trainer_dir.mkdir(parents=True)
        policy = manifest.get("training_resolution", DEFAULT_TRAINING_RESOLUTION); snapshot_assets = []
        for asset in manifest.get("assets", []):
            ...
            transform = _effective_transform(manifest, asset); dst = trainer_dir / filename
            transform_result = _materialize_transform(src, dst, transform)
            caption = str(asset.get("caption", "")).strip(); dst.with_suffix(".txt").write_text(caption + ("\n" if caption else ""), encoding="utf-8")
        ...
        run["dataset_path"] = str(trainer_dir); run["dataset_source_revision_path"] = str(source_dir); run["dataset_image_count"] = len(snapshot_assets); run.setdefault("config", {})["training_resolution"] = policy; _write_json(run_dir / "run.json", run)
        event = {"time": _now(), "type": "trainer_dataset_materialized", "revision": revision_id, "path": str(trainer_dir), "image_count": len(snapshot_assets), "training_resolution": policy, "transforms_materialized": True}
