...
def get_caption_template(project_id: str, revision_id: str):
    try:
        return {**caption_template_store.render(project_id, revision_id), "grammar_profiles": GRAMMAR_PROFILES}
    except FileNotFoundError as exc: raise _not_found(exc) from exc
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc

@router.put("/{project_id}/revisions/{revision_id}/caption-template")
def update_caption_template(project_id: str, revision_id: str, request: dict[str, Any]):
    try:
        values = request.get("values", request)
        caption_template_store.update(project_id, revision_id, grammar_profile=values.get("grammar_profile"), template_text=values.get("template_text"), validation=values.get("validation"), reset_template=bool(values.get("reset_template", False)))
        return {**caption_template_store.render(project_id, revision_id), "grammar_profiles": GRAMMAR_PROFILES}
    except FileNotFoundError as exc: raise _not_found(exc) from exc
...
def preview_caption_template(project_id: str, revision_id: str, request: dict[str, Any]):
    try:
        values = request.get("values", request)
        return {**caption_template_store.render(project_id, revision_id, state_override=values), "grammar_profiles": GRAMMAR_PROFILES}
    except FileNotFoundError as exc: raise _not_found(exc) from exc
    except (TypeError, ValueError) as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
